...
router = Router()
router.message.filter(F.from_user.id == OWNER_ID)  # bu routerdagi HAMMA handler faqat owner uchun


@router.message(Command("newsource"))
async def cmd_newsource(message: Message, command: CommandObject):
    """
    Foydalanish: /newsource bio Instagram bio linki
    -> code = "bio", label = "Instagram bio linki"
    """
    if not command.args:
...
async def handle_owner_reply(message: Message, bot: Bot):
    """
    Owner biror xabarga (yoki uning nusxasiga) reply qilganda,
    javobni asl yuboruvchiga uning xabariga thread qilib yuboradi.
    """
    replied_to_id = message.reply_to_message.message_id

    record = await queries.get_message_by_owner_msg_id(replied_to_id)
    if not record:
        # Bu owner panelidagi oddiy o'z-o'ziga yozgan xabar, botga aloqasi yo'q bo'lishi mumkin
        return
